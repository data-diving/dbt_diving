{%- macro get_nodes(select, resource_type='all', print_list=False) -%}
...
{%- for one_union in select.split(' ') -%}
    ...
    {%- for one_intersect in intersect_list -%}
        {%- if loop.index0 == 0 -%}
            {%- set ns.cur_node_list = dbt_diving.parse_one_condition(one_intersect, resource_type=resource_type) -%}
        {%- else -%}
            {%- set cur_node_list_next = dbt_diving.parse_one_condition(one_intersect, resource_type=resource_type) -%}
...
{%- macro parse_one_condition(condition, resource_type) -%}
...
{%- else -%}
    ...
    {%- else -%}
        {%- set nodes = all_nodes | selectattr('name', 'equalto', condition) | selectattr('resource_type', 'equalto', resource_type) | list -%}
    {%- endif -%}
